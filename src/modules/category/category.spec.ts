...
describe("Category Module", () => {
    ...
    describe("CRUD", () => {
        it("1. deve criar categoria com sucesso", async () => {
            (prisma.enterpriseCategory.create as jest.Mock).mockResolvedValue({ id: "cat1", name: "Free Fire" });

            const result = await createCategory("ent1", { name: "Free Fire" });

            expect(prisma.enterpriseCategory.create).toHaveBeenCalledWith({
        ...
        it("10. deve retornar conflito caso crie com nome duplicado na mesma empresa", async () => {
            ...
            (prisma.enterpriseCategory.create as jest.Mock).mockRejectedValue(p2002Error);
